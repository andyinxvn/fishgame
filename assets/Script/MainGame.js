...
cc.Class({
	...
	onLoad: function () {
		this.mywv.opacity = 0;
		this.mywv.removeComponent(cc.WebView);
		this.dialog.init();
		...
		if (check == null || cc.RedT.isSoundBackground()) {
			...
			cc.RedT.audio.audioBgID = cc.audioEngine.play(cc.RedT.audio.bg.clip, true, 1);
			console.log(cc.RedT.audio.bg.clip);
			console.log(cc.RedT.audio.audioBgID);
			console.log(cc.audioEngine.getState(cc.RedT.audio.audioBgID));
			console.log(cc.audioEngine.getCurrentTime(cc.RedT.audio.audioBgID));
			console.log(cc.audioEngine.getVolume(cc.RedT.audio.audioBgID));
		}
		...
		if (cc.RedT && !cc.RedT.IS_LOGIN) {
			...
			if (!!checkT && !!checkH) {
				let t = setInterval(function () {
					if (cc.RedT._socket && cc.RedT._socket.readyState === 1) {
	...
	onData: function (data) {
		if (this.dataOn) {
			...
			if (void 0 !== data.taixiu) {
				...
				if (this.iconTxtTai && data.taixiu.taixiu && data.taixiu.taixiu.red_tai) {
					helper.numberTo(this.iconTxtTai, helper.getOnlyNumberInString(this.iconTxtTai.string), data.taixiu.taixiu.red_tai, 4900, true);
				}
				if (this.iconTxtXiu && data.taixiu.taixiu && data.taixiu.taixiu.red_xiu) {
	...
	openBanca3() {
		if (cc.RedT && cc.RedT.IS_LOGIN) {
			this.mywv.addComponent(cc.WebView);
			this.mywv.opacity = 255;
			this.mywv.on('loaded', this.callback, this);

			// var scheme = "http://testkey";
			...
			// this.mywv.getComponent(cc.WebView).setJavascriptInterfaceScheme(scheme);
			// this.mywv.getComponent(cc.WebView).setOnJSCallback(jsCallback);

			this.mywv.getComponent(cc.WebView).url = `http://146.190.35.252/fish2/index.html?coin=${cc.RedT.user.red}&name=${cc.RedT.user.name}`;
		}
	},
	...
	signIn: function () {
		...
		console.log(this.EventsBanner);
		this.EventsBanner.active = true;
