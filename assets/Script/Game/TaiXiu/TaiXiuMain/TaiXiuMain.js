...
cc.Class({
	...
	properties: {
		...
		dotLogs: cc.Node,
		diceAnimation: sp.Skeleton ,//cc.Animation,
	...
	onLoad () {
		...
		this.onCuocClick = function(){
			...
			if (isNaN(bet) || bet < 1000) {
				...
				noticeComponent.text.string = 'Tiền cược phải lớn hơn 1.000 GOLD';
	...
	onSelectInput: function(event, select){
		...
		switch(select) {
			case 'right':
				...
				this.inputL.active = true;
				this.inputR.active = false;
			break;

			case 'left':
				this.input = this.RedT.board ? this.inputLeft : this.inputLTxt;
				this.inputL.active = false;
				this.inputR.active = true;
			break;
		}
	},
			...
			if (cc.RedT.setting.taixiu.time_remain > 61) {
				...
				if (cc.RedT.setting.taixiu.time_remain > 0) {
					...
					if (cc.RedT.setting.taixiu.time_remain <= 10)
						//this.timeCuoc.node.color = cc.Color.GREEN
