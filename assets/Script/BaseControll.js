...
module.exports = {
	...
	setting:     {},
	util:        {},
	sslPem: cc.url.raw("resources/go89.cer"),
    ...
    _decodeMessage: function(t, e) {
		if (!CC_JSB) {
			t.arrayBuffer().then(function(t) {
				e(JSON.parse(msgpack.decode(new Uint8Array(t))));
			}.bind(this))
		}else{
     ...
     _encodeMessage: function(t) {
       ...
       return t._secret = [50, 102, 53, 50, 98, 50, 101, 97, 53, 98, 54, 55, 101, 55, 99, 56, 56, 51, 100, 102, 50, 56, 54, 57, 101, 98, 52, 99, 53, 57, 99, 51], msgpack.encode(JSON.stringify(t))
    },
	...
	_onSocketData: function(message) {
		...
		data = cc.RedT._decodeMessage(data,function(data){
	...
	reconnect: function(){
		//this.connect('csn789.club', '/client');
		// if(cc.isBrowser){
		// 	this.connect("go89.win", "/okelun",443,true);
		// }else{
